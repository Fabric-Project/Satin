...
final class GeometryCoverageTests: XCTestCase {
    ...
    func testLineGeometry() throws {
        let image = try renderGeometry(size: [160, 160]) { _, camera in
    ...
    func testParametricGeometry() throws {
        let geometry = ParametricGeometry(rangeU: 0.0 ... 1.0, rangeV: 0.0 ... 1.0, resolution: simd_int2(24, 18)) { u, v in
            simd_float3(
                (u - 0.5) * 1.35,
                (v - 0.5) * 1.15,
    ...
    func testQuadGeometry() throws {
        try assertFlatGeometry(reference: "geometry-quad", geometry: QuadGeometry(size: 1.35))
    }
    ...
    func testSkyboxGeometry() throws {
        let image = try renderGeometry(size: [176, 176]) { device, camera in
    ...
    func testSphereGeometry() throws {
        try assertLitGeometry(reference: "geometry-sphere", geometry: SphereGeometry(radius: 0.62, angularResolution: 32, verticalResolution: 18))
    }
    ...
    func testTesselatedTextGeometry() throws {
        let image = try renderGeometry(size: [176, 176]) { _, camera in
            ...
            let mesh = Mesh(
                ...
                material: BasicDiffuseMaterial(color: simd_float4(0.95, 0.82, 0.3, 1.0), blending: .disabled, hardness: 0.75)
            )
            ...
            return makeScene(mesh, extras: [light])
        }
    ...
    func testTextGeometry() throws {
        ...
        let image = try renderGeometry(size: [176, 144]) { _, camera in
            ...
            let mesh = Mesh(
                ...
                material: BasicColorMaterial(color: simd_float4(0.96, 0.84, 0.3, 1.0), blending: .disabled)
            )
    ...
    func testTriangleGeometry() throws {
        try assertFlatGeometry(reference: "geometry-triangle", geometry: TriangleGeometry(size: 1.35))
    }
...
private func assertLitGeometry(
    ...
    let image = try renderGeometry(size: [176, 176]) { _, camera in
        ...
        return makeScene(mesh, extras: [light, fill])
    }
...
private func makeScene(_ primary: Object, extras: [Object] = []) -> Object {
    let scene = Object(label: "Scene")
